...
if __name__ == "__main__":
    ...
    for epoch in range(40):
        ...
        if best_loss > np.mean(losses):
            ...
            torch.save(model.state_dict(), "model_{}_STAWnet.pth".format(dataset))

    model.load_state_dict(torch.load("model_{}_STAWnet.pth".format(dataset)))
    model.eval()
    t = 0
    ...
    preds = np.concatenate(preds)
    np.save(dataset+'preds_STAWnet', preds, allow_pickle=True)
