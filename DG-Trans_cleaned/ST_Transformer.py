...
from GCN_models import GCN, GCN_evolution, I_GCN_evolution, IGNN
...
class STransformer(nn.Module):
    def __init__(self, embed_size, time_num, heads, adj, dataset, dropout, forward_expansion):
        ...
        if self.dataset == 'pems08_spatial2':
            H, A = adj
            adj = A
            self.adj = H
            self.D_S = nn.Parameter(adj)  # act as position encoding
        elif dataset == 'pems08_implicit':
            self.adj = adj
            self.D_S = nn.Parameter(adj.to_dense())  # act as position encoding
        else:
            if self.dataset == 'pems08_static':
                H, A = adj
                adj = torch.mm(torch.mm(H, A), H.T)
        ...
        # 调用GCN
        if self.dataset == 'pems08_spatial2':
            self.gcn = GCN_evolution(embed_size, embed_size*2, embed_size, dropout)
        if self.dataset == 'pems08_implicit':
            self.gcn = IGNN(embed_size, embed_size*2, embed_size, adj.shape[0], dropout)
        else:
...
class STransformer2(nn.Module):
    def __init__(self, embed_size, time_num, heads, adj, dataset, dropout, forward_expansion):
        super(STransformer2, self).__init__()
        # Spatial Embedding
        self.dataset = dataset

        if dataset == 'pems08_co-evo':
            self.H, self.adj = adj
            self.H = sp.coo_matrix(self.H.cpu().numpy())
            self.H = sparse_mx_to_torch_sparse_tensor(self.H).float()
        ...
        # 调用GCN
        self.gcn = IGNN(embed_size, embed_size * 2, embed_size, adj.shape[0]*time_num, dropout)
        self.norm_adj = nn.InstanceNorm2d(1)  # 对邻接矩阵归一化
    ...
    def forward(self, value, key, query):
        ...
        # with torch.autograd.profiler.profile(use_cuda=True) as prof:
            # A = torch.block_diag(*[self.adj for i in range(T)])
            # A = sp.coo_matrix(A.cpu().numpy())
            # A = sparse_mx_to_torch_sparse_tensor(A).float()
            # X_G = self.gcn(query.view(-1, C), A).view(N, T, C)
        X_G = self.gcn(query[:, -1, :], self.adj).unsqueeze(1).expand(-1, T, -1)
        # print(prof.key_averages().table(sort_by='cuda_time_total'))
        # prof.export_chrome_trace('trace/trace-stack_diag.json')
