...
if __name__ == "__main__":
    ...
    for epoch in range(100):
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            out, adj = model(x, t) # ignore the time feature
        ...
        # if best_train > loss:
        #     torch.save(model.state_dict(), "model_{}_distadj.pth".format(dataset))
        #     best_train = loss
        ...
        if best_loss > np.mean(losses):
            ...
            torch.save(model.state_dict(), "model_{}_ST_distadj.pth".format(dataset))

    model.load_state_dict(torch.load("model_{}_ST_distadj.pth".format(dataset)))
    model.eval()
    ...
    for iter, (x, y) in enumerate(dataloader['test_loader'].get_iterator()):
        ...
        # t, r, c = y[:, 0].round().type(torch.int).item(), y[:, 1].round().type(torch.int).item(), y[:, 2].round().type(torch.int).item()
        out, adj = model(x, t)
        # atts.append(adj[:,:,6].squeeze().detach().cpu().numpy())
        out = out.detach().cpu().numpy()
        preds.append(out)

    preds = np.concatenate(preds)
    preds = preds[:dataloader['y_test'].shape[0]]
    print('MAE:', MAE_np(dataloader['y_test'], preds), 'MAPE:', SMAPE_np(dataloader['y_test'], preds),
          'RMSE:', RMSE_np(dataloader['y_test'], preds), 'R2:', R2_np(dataloader['y_test'], preds))
    ...
    # np.save(dataset+'atts_ST', atts, allow_pickle=True)
